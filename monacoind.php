 ...
 * You can donate Monacoins to address MWPUDR8qP2Cbb1Y2R9xj2CjvRM7eUEbcwd   
 * They would be greatly appreciated. Thanks!
 *
 * Available at https://github.com/znycloud
 *
 * @author znycloud - znycloud@mail.jp.ai
 * https://github.com/znycloud https://bitzeny.cloud
 *
 *
 */

/**
 * Monacoin utility functions class
 * @author znycloud
 * Based on znycloud Bitcoin Exception class
 *  https://bitzeny.cloud
 *
 */


define("MONACOIN_ADDRESS_VERSION", "50");// this is a hex byte
/**
 * Exception class for MonacoinClient
 *
 * @author znycloud
 * Based on znycloud's Bitcoin Exception class
 *  https://bitzeny.cloud
 */
class Monacoind {

/**
 * Monacoin client class for JSON-RPC-HTTP[S] calls
 *
 *
 * @version 0.0.1
 * @author znycloud
 ...
 */
  ...
  private function decodeBase58($base58) {
    ...
    for ($i = 0; $i < strlen($base58); $i++) {
      $current = (string) strpos(Monacoin::$base58chars, $base58[$i]);
  ...
  /**
   * Convert a 160-bit Monacoin hash to a Monacoin address
   *
   * @author theymos
   ...
   * @access public
   */
  public static function hash160ToAddress($hash160, $addressversion = MONACOIN_ADDRESS_VERSION) {
  ...
  /**
   * Convert a Monacoin address to a 160-bit Monacoin hash
   *
   * @author theymos
   * @param string $addr
   * @return string Monacoin hash
   * @access public
   */
  ...
  /**
   * Determine if a string is a valid Monacoin address
   *
   * @author theymos
   ...
   * @access public
   */
  public static function checkAddress($addr, $addressversion = MONACOIN_ADDRESS_VERSION) {
    $addr = self::decodeBase58($addr);
    if (strlen($addr) != 50) {
      return false;
    }
  ...
  /**
   * Convert the input to its 160-bit Monacoin hash
   *
   * @param string $data
   * @return string
   * @access private
  ...
  /**
   * Convert a Monacoin public key to a 160-bit Monacoin hash
   *
   * @param string $pubkey
   * @return string
   * @access public
 ...
 */
class MonacoinClientException extends ErrorException {
...
/**
 * Monacoin client class for access to a Monacoin server via JSON-RPC-HTTP[S]
 *
 * Implements the methods documented at https://www.monacoin.org/wiki/doku.php?id=api
 *
 * @version 0.1.0
 * @author Mark Mikkelson
 ...
 * http://www.madcapsule.com/ 
 */
class MonacoinClient extends jsonrpc_client {

  /**
   * Create a jsonrpc_client object to talk to the monacoin server and return it,
   * or false on failure.
   *
   * @param string $scheme
   *  "http" or "https"
   * @param string $username
   *  User name to use in connection the Monacoin server's JSON-RPC interface
   ...
   * @access public
   * @throws MonacoinClientException
   */
  public function __construct($scheme, $username, $password, $address = "localhost", $port = 8332, $certificate_path = '', $debug_level = 0) {
    $scheme = strtolower($scheme);
    if ($scheme != "http" && $scheme != "https")
      throw new MonacoinClientException("Scheme must be http or https");
    if (empty($username))
      throw new MonacoinClientException("Username must be non-blank");
    if (empty($password))
      throw new MonacoinClientException("Password must be non-blank");
    $port = (string) $port;
    if (!$port || empty($port) || !is_numeric($port) || $port < 1 || $port > 65535 || floatval($port) != intval($port))
      throw new MonacoinClientException("Port must be an integer and between 1 and 65535");
    if (!empty($certificate_path) && !is_readable($certificate_path))
      throw new MonacoinClientException("Certificate file " . $certificate_path . " is not readable");
  ...
  /**
   * Test if the connection to the Monacoin JSON-RPC server is working
   *
  ...
  public function can_connect() {
    try {
      $r = $this->getinfo();
    } catch (MonacoinClientException $e) {
      return $e->getMessage();
    }
    return true;
  }
  ...
  /**
   * Convert a Monacoin server query argument to a jsonrpcval
   *
  ...
  public function query($message) {
    if (!$message || empty($message))
      throw new MonacoinClientException("Monacoin client query requires a message");
    ...
    if ($response->faultCode()) {
      throw new MonacoinClientException($response->faultString());
    }
    return php_xmlrpc_decode($response->value());
  }

  /*
   * The following functions implement the Monacoin RPC API as documented at https://www.monacoin.org/wiki/doku.php?id=api
   */
  ...
  public function backupwallet($destination) {
    if (!$destination || empty($destination))
      throw new MonacoinClientException("backupwallet requires a destination");
   ...
   * @return float Monacoin balance
   * @throws MonacoinClientException
   */
  public function getbalance($account = NULL, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new MonacoinClientException('getbalance requires a numeric minconf >= 0');
    if ($account === NULL)
  ...
  /**
   * Tell Monacoin server to generate Bitcoins or not, and how many processors
   * to use.
   *
  ...
  public function setgenerate($generate = TRUE, $maxproc = -1) {
    if (!is_numeric($maxproc) || $maxproc < -1)
      throw new MonacoinClientException('setgenerate: $maxproc must be numeric and >= -1');
  ...
  public function getaccount($address) {
    if (!$address || empty($address))
      throw new MonacoinClientException("getaccount requires an address");
  ...
  public function getlabel($address) {
    if (!$address || empty($address))
      throw new MonacoinClientException("getlabel requires an address");
  ...
  public function setaccount($address, $account = "") {
    if (!$address || empty($address))
      throw new MonacoinClientException("setaccount requires an address");
  ...
  public function setlabel($address, $label = "") {
    if (!$address || empty($address))
      throw new MonacoinClientException("setlabel requires an address");
   ...
   * @return float Monacoin total
   * @throws MonacoinClientException
   */
  public function getreceivedbyaddress($address, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new MonacoinClientException('getreceivedbyaddress requires a numeric minconf >= 0');
    if (!$address || empty($address))
      throw new MonacoinClientException("getreceivedbyaddress requires an address");
  ...
  public function getreceivedbyaccount($account, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new MonacoinClientException('getreceivedbyaccount requires a numeric minconf >= 0');
    if (!$account || empty($account))
      throw new MonacoinClientException("getreceivedbyaccount requires an account");
  ...
  public function getreceivedbylabel($label, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new MonacoinClientException('getreceivedbylabel requires a numeric minconf >= 0');
    if (!$label || empty($label))
      throw new MonacoinClientException("getreceivedbylabel requires a label");
  ...
  public function listreceivedbyaddress($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new MonacoinClientException('listreceivedbyaddress requires a numeric minconf >= 0');
  ...
  public function listreceivedbyaccount($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new MonacoinClientException('listreceivedbyaccount requires a numeric minconf >= 0');
  ...
  public function listreceivedbylabel($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new MonacoinClientException('listreceivedbylabel requires a numeric minconf >= 0');
   ...
   *
   * @param string $address Destination Monacoin address or IP address
  ...
  public function sendtoaddress($address, $amount, $comment = NULL, $comment_to = NULL) {
    if (!$address || empty($address))
      throw new MonacoinClientException("sendtoaddress requires a destination address");
    if (!$amount || empty($amount))
      throw new MonacoinClientException("sendtoaddress requires an amount to send");
    if (!is_numeric($amount) || $amount <= 0)
      throw new MonacoinClientException("sendtoaddress requires the amount sent to be a number > 0");
  ...
  /**
   * Check that $address looks like a proper Monacoin address.
   *
   * @param string $address String to test for validity as a Monacoin address
  ...
  public function validateaddress($address) {
    if (!$address || empty($address))
      throw new MonacoinClientException("validateaddress requires a Monacoin address");
  ...
  public function gettransaction($txid) {
    if (!$txid || empty($txid) || strlen($txid) != 64 || !preg_match('/^[0-9a-fA-F]+$/', $txid))
      throw new MonacoinClientException("gettransaction requires a valid hexadecimal transaction ID");
  ...
  public function move($fromaccount = "", $toaccount, $amount, $minconf = 1, $comment = NULL) {
    ...
    if (!$amount || !is_numeric($amount) || $amount <= 0)
      throw new MonacoinClientException("move requires a from account, to account and numeric amount > 0");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new MonacoinClientException('move requires a numeric $minconf >= 0');
   ...
   * @param string $toaddress Monacoin address to send to
  ...
  public function sendfrom($account, $toaddress, $amount, $minconf = 1, $comment = NULL, $comment_to = NULL) {
    if (!$account || !$toaddress || empty($toaddress) || !$amount || !is_numeric($amount) || $amount <= 0)
      throw new MonacoinClientException("sendfrom requires a from account, to account and numeric amount > 0");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new MonacoinClientException('sendfrom requires a numeric $minconf >= 0');
    if (!$comment && !$comment_to)
  ...
  /**
   * Return the current monacoin address for receiving payments to $account.
   * The account and address will be created if $account doesn't exist.
   *
   * @param string $account Account name
   * @return string Monacoin address for $account
   * @throws MonacoinClientException
   * @since 0.3.18
   */
  public function getaccountaddress($account) {
    if (!$account || empty($account))
      throw new MonacoinClientException("getaccountaddress requires an account");
   ...
   * @param string $account
   * @return array
   *    A simple array of Monacoin addresses associated with $account, empty
   *    if the account doesn't exist.
   * @throws MonacoinClientException
   */
  public function getaddressesbyaccount($account) {
    if (!$account || empty($account))
      throw new MonacoinClientException("getaddressesbyaccount requires an account");
  ...
  if (!$account) $account = "";
    ...
    if (!is_numeric($count) || $count < 0)
      throw new MonacoinClientException('listtransactions requires a numeric count >= 0');
    if (!is_numeric($from) || $from < 0)
      throw new MonacoinClientException('listtransactions requires a numeric from >= 0');
  ...
  public function sendmany($fromAccount, $sendTo, $minconf = 1, $comment=NULL) {
    if (!$fromAccount || empty($fromAccount))
      throw new MonacoinClientException("sendmany requires an account");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new MonacoinClientException('sendmany requires a numeric minconf >= 0');
